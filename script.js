...
$('.contentbox').hide();
	//used to be infobox first$('#titlebox').fadeIn('fast');
	//$('#infobox').fadeIn('slow');
	//Now the menu comes up first
$('#menubox').fadeIn('slow');
$('.item').hide();
$('#item1').fadeIn('slow');
$('#menul1').css('color','black');
$('form').hide();
$('#menutab a').css('text-shadow', '2px 2px 17px darkgray'); // make menu text brigter
$('#ourstorytab a').css("text-shadow", '2px 2px 17px black'); // make sure our story text is balck ( for some reason it is brighter.

$('#infotab a').css("text-shadow", '2px 2px 17px black');// make sure event info text isn't bright
var tabnotclicked=true
	if(tabnotclicked){
	$('#menutab a').css('background-color', '#3d63b8'); // make background blue so it is clear we are in this tab
	} 
$('#buttons').click(function() {
	$('#menutab a').css('background-color', 'none'); // make background blue so it is clear we are in this tab
...
//		});
	...
	$('#menutab').click(function() {
		...
		$('#menutab').css('background-color', '#3d63b8'); // make background blue so it is clear we are in this tab
		// make other menu button ( aka tab) backgrounds no background
		$('#infotab').css('background-color', 'none');
		$('#ourstorytab').css('background-color', 'none');

		tabnotclicked= false // if we are in this function than it means a tab has been clicked and the blue background at the beginning should be undone
		
		// eventually it would be nice if button changed color when it was clicked $('#menutab').css('background', '#3d63b8');
	});
		
		//infolink // on april 2018 I am not sure what this is or if it does anything.
	$('#infolink').click(function() {
		$('.contentbox').fadeOut('fast');
		$('#infobox').fadeIn('slow');
		
		
		$('#infotab').css('background-color', '#3d63b8'); // make background blue so it is clear we are in this tab
		// make other menu button ( aka tab) backgrounds no background
		$('#menutab').css('background-color', 'none');
		$('#ourstorytab').css('background-color', 'none');
    ...
    $('#ourstorytab').click(function() {
		$('#ourstorytab').css('background-color', '#3d63b8'); // make background blue so it is clear we are in this tab
		// make other menu button ( aka tab) backgrounds no background
		$('#infotab').css('background-color', 'none');
		$('#menutab').css('background-color', 'none');
	...
	$('#menul13').mouseenter(function() {
      	 	 $('#item13').fadeIn('slow');
		 
		
  	});
   	$('#menul13').mouseleave(function() {
      		 $('#item13').fadeOut('slow');
	...
	$('#menul14').mouseenter(function() {
      	 	 $('#item14').fadeIn('slow');
		 
		
  	});
   	$('#menul14').mouseleave(function() {
      		 $('#item14').fadeOut('slow');
		  
  	});
	
	
	$('#menul15').mouseenter(function() {
      	 	 $('#item15').fadeIn('slow');
		 
		
  	});
   	$('#menul15').mouseleave(function() {
      		 $('#item15').fadeOut('slow');
